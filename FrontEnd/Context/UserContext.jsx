...
const UserProvider = ({ children }) => {
    ...
    if (token) {
      ...
      try {
        const res = await baseURL.get(
          "/checkout/getUserPayments/" + decoded._id
        );
  ...
  const fetchBoatDetails = async () => {
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to fetch boat details";
  ...
  const fetchBoatDetailsById = async (id) => {
    try {
      ...
      throw new Error(
        error.response.data.message || "Error fetching boat details"
      );
    }
  };
