...
const ConditionPopup = ({ condition, onClose }) => {
  ...
  const [conditionName, setConditionName] = useState(condition?.conditionName || '');
  const [description, setDescription] = useState(condition?.description || '');

  const handleSave = async () => {
    const conditionData = { conditionId: condition._id, conditionName, description };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-1/3">
        <h2 className="text-2xl mb-4">{condition ? 'Edit Condition' : 'Add Condition'}</h2>
        <input
          type="text"
