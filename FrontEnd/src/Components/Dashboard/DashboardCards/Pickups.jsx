...
const Pickups = ({ title, opt3 }) => {
  ...
  const [bookings, setBookings] = useState({ today: [], tomorrow: [], dayAfterTomorrow: [] });
      ...
      {loading ? (
        ...
        selectedBookings.length > 0 ? (
          selectedBookings.map((booking, index) => (
            <div key={index} className="border-l-[1.5px] relative border-[#A8AAAE]">
              ...
              <div className="flex gap-2 justify-between flex-wrap items-start mt-4 p-6">
                <div>
                  ...
                  <p className="text-base text-[--primary-color]">${booking.amount}</p>
                </div>
