...
const Booking2 = () => {
  ...
  const { myBookings, fetchMyBookings, fetchBoatDetailsById } = useContext(UserContext);
  // fetch bookings
  useEffect(() => {
    if (!myBookings) {
      fetchMyBookings()
      ...
      <div className=" flex flex-col md:flex-row md:space-x-[2%] mt-[2%] space-y-[5%] md:space-y-[0%]">
        <div className="md:w-[35%] bg-white rounded-xl shadow-checkout mb-[1%]">
          ...
          <div className="py-9 px-12 flex justify-center flex-col text-[#383838]">
            <img src={boatDetails && boatDetails?.boatImages[0]?.avatar} alt="" className="md:w-64" />
            ...
            <div className=" flex flex-col gap-1 mt-[3%]">
              ...
              <p>
                Model:{' '}
                <span className=" text-[#676767] font-normal">{boatDetails?.boat?.model}</span>
              </p>
              <p>
                Engine Power:{' '}
                <span className=" text-[#676767] font-normal">{boatDetails?.boat?.totalEnginePowerHP} HP</span>
              </p>
              <p>
                Fuel Capacity:{' '}
                <span className=" text-[#676767] font-normal">{boatDetails?.boat?.fuelTankLiters} L</span>
              </p>
              <p>
                Water Capacity:{' '}
                <span className=" text-[#676767] font-normal">{boatDetails?.boat?.waterTankLiters} L</span>
              </p>
              <p>
                Date:{' '}
                <span className=" text-[#676767] font-normal">{formattedDate}</span>
              </p>
              {/* <p>
                Payment Status:{' '}
          ...
          <div className="border-t border-b border-[#CBA557] py-9 px-12 leading-7">
            ...
            <div className=" flex justify-between w-[90%] mt-[1%]">
              <p className=" text-lg font-normal text-[#676767]">Payment Status</p>
              <p className=" text-lg font-medium text-[#383838]">{thisBooking?.paymentStatus}</p>
            </div>
            <div className=" flex justify-between w-[90%] mt-[1%]">
              <p className=" text-lg font-normal text-[#676767]">Duration</p>
              <p className=" text-lg font-medium text-[#383838]">{thisBooking?.rateType}</p>
            </div>
          </div>
